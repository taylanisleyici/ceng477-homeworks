...
void renderImageThread(const Camera &camera, const Scene &scene, BVHNode *root, unsigned char *image, int height, int width, unsigned int modulo)
{
    ...
    for (size_t j = 0; j < height; j++)
    {
        for (size_t k = 0; k < width; k++)
        {
            if (k % 4 == modulo)
            {
                ...
                image[i + 1] = pixelValues.y;
                image[i + 2] = pixelValues.z;
            }
        }
    }
}
...
void renderImageFromCamera(const Camera &camera, const Scene &scene, BVHNode *root)
{
    ...
    {
        threads[i] = std::thread(renderImageThread, camera, scene, root, &image[0], height, width, i);
    }
